...
rechercheTabou::rechercheTabou(int nbiter, int dt, int nv, char *nom_fichier)
{
    ...
    courant = new solution(nv, les_distances);
...
{
    return list_tabou[i][j] < iter_courante;
}
    ...
    for (int i = 0; i < taille_solution; i++)
    {
        ...
        for (int j = i + 1; j < taille_solution; j++)
        {
            if (((i != 0) || (j != taille_solution - 1)) && ((i != 0) || (j != taille_solution - 2)))
            {
                ...
                if (courant->fitness < best_vois && nonTabou(i, j))
                {
...
solution *rechercheTabou::optimiser(bool opt2)
{
    ...
    for (iter_courante = 0; iter_courante < nbiterations; iter_courante++)
    {
        ...
        cout << iter_courante << "\t" << courant->fitness << "\t" << best_eval << endl;
    }
...
void rechercheTabou::voisinage_2opt(int &best_i, int &best_j)
{
    ...
    // on sélectionne une première ville pour le mouvement
    for (int i = 0; i < taille_solution - 1; i++)
    {
        // on sélectionne une seconde ville pour le mouvement
        for (int j = i + 1; j < taille_solution; j++)
        {
            if (((i != 0) || (j != taille_solution - 1)) && ((i != 0) || (j != taille_solution - 2)))
            {
